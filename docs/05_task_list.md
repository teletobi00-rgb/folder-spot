...
> ✅ 2026-06-12 완료. WinSxS 28,514항목 나열 47ms + 정렬 358ms(백그라운드). 테스트 118개.
> 비고: 아이콘 로드는 전용 STA 스레드(StaWorker) 경유 — 스레드풀(MTA)에서 일부 셸 확장이 조용히 실패하는 문제 회피.
> 알려진 한계(후속 Phase): desktop.ini 커스텀 폴더 아이콘 미지원, 정렬 방향 표시 글리프 없음, 다중 선택 바인딩은 Phase 2에서.
...
> ✅ 2026-06-12 완료 — **v0.1 데일리 드라이버 마일스톤 달성.** 테스트 286개.
> 구현: F5/F6(반대 페인 복사·이동, DropRules 가드+상태 메시지), Tab(페인 전환+포커스 추적), Ctrl+U(페인 스왑 — 중간상태 미노출 일괄 통지),
> Ctrl+←/→(반대 페인에 열기), 중앙 키맵(KeyMap+keymap.json 오버라이드, 코드 생성 InputBindings, 수정자 없는 Tab 지원 자체 GestureParser),
> 즐겨찾기(파일+폴더, 사이드바 섹션, 드래그 추가, 우클릭 제거/순서, JSON 영속), 목록 우클릭 '즐겨찾기에 추가'.
> 이월: 즐겨찾기 드래그 재정렬(메뉴로 대체), 키맵 재바인딩 UI(v1.x).
...
> ✅ 2026-06-12 완료. 테스트 325개(실제 셸 엔진 통합 검증 포함).
> 구현: 직렬 작업 큐(Channel 워커) + 하단 큐 패널(%, ETA, 일시정지/재개/취소), 충돌 사전 스캔 → 커스텀 다이얼로그
> (덮어쓰기/건너뛰기/둘 다 유지 + 모두 적용) → 결정 그룹별 셸 플래그 실행, Ctrl+Z Undo(복사→생성본 삭제,
> 이동→되돌려 이동, 휴지통 삭제→$R 물리 항목 복귀 — PostDeleteItem이 주는 휴지통 경로 활용).
> 기술 노트: 일시정지=Vanara 이벤트 핸들러에서 게이트 블로킹, 취소=COMException(COPYENGINE_E_USER_CANCELLED)
> + Control 상태 후검사(엔진이 항목 취소를 '건너뜀'으로 집계하는 경우 보정). 셸 진행/충돌 UI는 Silent로 전부 대체.
> 알려진 한계: $R 복원 시 $I 메타데이터 고아(휴지통 비우기로 정리), 속도는 %/s 기반 ETA(바이트 속도는 후속).
...
- **네이티브 컨텍스트 메뉴 out-of-proc 호스팅 (우선순위 상향, 2026-06-12)**: 이 머신에서 특정 셸 확장이
  비탐색기(.NET) 프로세스 내 QueryContextMenu 실행만으로 힙 손상(0xc0000374) 유발 — 진단 스트레스 테스트로 확정
  (해제 순서 교정·완전 누수 변형 모두 크래시 → 실행 자체가 손상 원인). 현재 대응: 자체 메뉴 기본 + "Windows 메뉴(불안정)" 옵트인.
  근본 해결: 헬퍼 프로세스 메뉴 호스팅 + 문제 확장 자동 식별(핸들러별 순차 QueryContextMenu, 크래시 직전 로깅으로 범인 특정).
